...
export const fetchCurrentWeather = async (lat, lon, date) => {
  const params = new URLSearchParams({
    ...
    hourly: [
      ...
      "visibility","wind_speed_10m","uv_index","weather_code",
...
export const fetchHistoricalAirQuality = async (lat, lon, startDate, endDate) => {
  const params = new URLSearchParams({
    latitude: lat,
    longitude: lon,
...
export const aggregateAQDaily = (aqData) => {
  ...
  return Object.entries(daily).map(([date, v]) => ({
    ...
    pm2_5: v.pm2_5.length ? +(v.pm2_5.reduce((a, b) => a + b, 0) / v.pm2_5.length).toFixed(1) : null,
  }));
};